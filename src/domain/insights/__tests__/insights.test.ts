...
test('Insights — Daily Aggregation computes usage, sessions, cooldowns and access leases', () => {
  ...
  assert.equal(summary.scheduledRoutineMinutes, 90 + 120); // 90m morning + 120m evening = 210m
  assert.ok(summary.observedProtectedMinutes > 0);
...
test('Insights — Empty History returns clean empty summary', () => {
  ...
  assert.equal(summary.observedProtectedMinutes, 0);
  ...
  const summaries: DailyRhythmSummary[] = [
    {
      ...
      scheduledRoutineMinutes: 210,
      observedProtectedMinutes: 150,
      accessLeaseCount: 0,
...
test('Insights — Compaction purges raw events > 14 days and summaries > 90 days', () => {
  ...
  const summaries = [
    { dateKey: getLocalDateKey(now - 30 * dayMs), riskUsageSecondsByGroup: {}, sessionCountByGroup: {}, cooldownCountByGroup: {}, cooldownMinutesByGroup: {}, scheduledRoutineMinutes: 0, observedProtectedMinutes: 0, accessLeaseCount: 0, longestRiskSessionSeconds: 0 },
    { dateKey: getLocalDateKey(now - 100 * dayMs), riskUsageSecondsByGroup: {}, sessionCountByGroup: {}, cooldownCountByGroup: {}, cooldownMinutesByGroup: {}, scheduledRoutineMinutes: 0, observedProtectedMinutes: 0, accessLeaseCount: 0, longestRiskSessionSeconds: 0 },
  ];
