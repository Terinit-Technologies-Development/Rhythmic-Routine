...
| **Cross-Platform** | Release Build Profiles (`eas.json`) | `source-implemented` | Private/internal Android build channels: `development` (developer client), `preview` (internal QA APK), `production` (internal stable APK); source-controlled versioning (`appVersionSource: local`). No store submission profile. |
...
| **Android** | Physical Device Hardware Qualification | `physical-device-verified` | 100% of owner physical device checklist items passed across Pass 04C and Pass 04D. Verified on physical hardware. |
